<template>
	<div class="panel panel-primary" style="margin-top: 10px">
		<div class="panel-heading">Типы мероприятий</div>
		<div class="panel-body">
			<router-link role="button" class="btn btn-lg btn-block btn-default" :to="{name: 'editType', params: {id: 0}}">
				Добавить
			</router-link>
			<a href="/">
		...
		<table class="table table-striped">
			...
			<tr v-for="item in type" :key=item.id>
				<td>{{item.id}}</td>
				<td>{{ item.name }}</td>
				<td><router-link role="button" class="btn btn-sm btn-link" :to="{name: 'editType', params: {id: item.id}}">
